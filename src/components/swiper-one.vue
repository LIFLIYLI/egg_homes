...
export default {
  ...
  methods: {
    move(direction) {   
      //控制点击上下按钮式，当前显示的图片index,确保在列表length中  
      direction == 1?this.num++:this.num-- 
      if(this.num<0)this.num=this.sliders.length-1
      if(this.num>=this.sliders.length)this.num=0
      //ul的偏移量，每次点击切换，让偏移量对应的加或者减一张图片的width，形成切换
      this.distance = this.distance -direction*this.width;
      console.log(this.distance)
      if(this.distance ==0)this.distance=this.sliders.length*this.width*direction
